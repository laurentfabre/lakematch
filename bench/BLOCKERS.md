...
| ZR-7 | APX 0.3.8 scaffold builds below the file-size limit, but ongoing maintenance is unconfirmed. | A maintained APX source/version, or an explicit decision to proceed with 0.3.8 while that requirement remains unresolved. No AppKit/custom-UI substitution was made. |
| ZR-8 / ENV | Delegated app-to-Genie Conversation API is untested; the arbitration app prerequisite is unresolved. | Resolve the app prerequisite, then prove the ten versioned question checks through an actual delegated app session. Listing spaces is not capability proof. |
...
No remote experiment remains active. The owned warehouse is STOPPED, the
