...
Shipped defaults are not yet reconciled with the complete pipeline winner. The existing GBT/Levenshtein values agree with this completed stage; candidate and clustering selection, startup-inclusive latency, scale and final confirmation remain open; the shipped token default still needs reconciliation with the selected IDF-only set. No accepted pointer or UC alias was promoted by this checkpoint.
