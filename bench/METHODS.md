...
Shipped defaults are not yet reconciled with the complete pipeline winner. The existing GBT/Levenshtein values agree with this completed stage; candidate and clustering validation selection are complete; both frozen FEBRL quality/latency gates passed, and the scale ladder is in progress; the shipped token default still needs reconciliation with the selected IDF-only set. No accepted pointer or UC alias was promoted by this checkpoint.
