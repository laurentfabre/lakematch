...
| ZR-6 | Bundle validates and an actual FEVM serverless run completes. SDP holds candidates, features, scoring and links; training/clustering are tasks. DQX and native quality engines agree on seeded bad rows. Each FEBRL4 variant is within 0.01 absolute F1 of its frozen local reference. Default config and all-paid-features-off config both deploy/run; query profiles support the Photon report. | In progress | Corrected DQX run `299080784128178` passes: exact local F1, seeded quarantine, terminal job and IDLE pipeline. Native/all-paid-off run `622968507486615` also passes with exact local parity. Train/cluster/Delta fixture run `180393856450383` is active; measured per-stage Photon operator profiles remain pending. [Remote evidence](bench/SERVERLESS.md), [plan](bench/SERVERLESS_ADAPTER_PLAN.md). |
