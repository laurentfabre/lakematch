...
| ZR-6 | Bundle validates and an actual FEVM serverless run completes. SDP holds candidates, features, scoring and links; training/clustering are tasks. DQX and native quality engines agree on seeded bad rows. Each FEBRL4 variant is within 0.01 absolute F1 of its frozen local reference. Default config and all-paid-features-off config both deploy/run; query profiles support the Photon report. | In progress | Corrected DQX run `299080784128178` passes: exact local F1, seeded quarantine, terminal job and IDLE pipeline. Native/all-paid-off run `622968507486615` is active. Train/cluster/Delta fixture and measured Photon operator profiles remain pending. [Remote evidence](bench/SERVERLESS.md), [plan](bench/SERVERLESS_ADAPTER_PLAN.md). |
...
- Terminal profile capture passed after correcting the raw API's 250-event page limit; the rejected request is retained. All 16 completed flows were retrieved, but query history returned zero matching queries. Photon operator fallback profiles/task-time shares and billing remain missing. [Photon report](bench/PHOTON.md), [capture receipt](experiments/serverless-dqx-profile.json). The planned iteration-5 fixture additionally exports committed snapshots for independent journal replay after FEVM expiry; it is still unexecuted.
