...
| ZR-1 | Installable package, YAML config and CLI; native quality checks split valid/quarantined rows with reasons; deterministic candidates/features/matcher/decision produce links. Tests pass on local Spark and local Spark Connect, including all-paid-features-off offline execution. Implement capability-aware materialization and phase verifiers. | Passed | [Current core evidence](bench/ENGINE.md): 130 identical tests pass on classic Spark and local Spark Connect, no skips; package/private-repository, offline CLI and development smoke pass. Source `9babf294d0b96`; `bash verify_zr.sh 1` exits 0. |
| ZR-2 | Feature families cover every configured field type. Default matching plans have no `PythonUDF`, `BatchEvalPython` or `ArrowEvalPython`; optional Jaro-Winkler and affine-gap features are gated. Ablations cover FEBRL4-half-unmatched, BPID, Leipzig Affiliations and Abt-Buy, including local embeddings for organisation/title, accuracy and preprocessing cost. | Passed at iteration 6 | [Ablations](bench/ABLATION.md), [feature contract](bench/FEATURES.md), [run index](bench/ablation_index.json). All four unchanged offline fixtures pass on current source, including individual field-family removals, native plans, optional UDFs and actual local MiniLM execution. No confirmation scoring. `bash verify_zr.sh 2` exits 0. |
| ZR-3 | Reproducible harness runs every brief corpus, compares every required method on validation and ships the documented winners. On held-out FEBRL4-half-unmatched: F1 >= 0.97 with all fields and >= 0.96 with SSN hidden; each local end-to-end run is under 60 seconds including Spark startup. Publish candidate recall, confidence intervals, baselines and cost. Include the scale evidence specified below. | Blocked after iteration 8 | [Benchmarks](bench/BENCHMARKS.md), [exact replay audit](bench/compatibility_index.json), [CLI execution](bench/campaign_execution.json). All eight frozen replays have zero deltas and identical decisions; FEBRL F1 0.98888 / 0.98683 at 30.67s / 27.31s. Original diagnostic and clustering stages complete. Million-record heap failure blocks complete CLI acceptance; no ninth iteration authorized. |
...
## § 8 — Historical campaign checkpoints

These chronological entries retain earlier states and failures. The acceptance table and latest checkpoint below describe the current state.
...
- Corrected ZR-6 iteration-4 run `299080784128178` passed all three tasks in 1,489s including runner deployment/capture. Both confirmation F1 deltas are exactly zero (0.9888776542 / 0.9868287741); three seeded rows per side are quarantined for each variant. Job terminal and pipeline IDLE verified, no cleanup errors. Native/all-paid-off run `622968507486615` is now active, local experiment `20260920T033318Z-serverless-frozen-native-0277ba`, under the same bounds. [Sealed remote audit](bench/SERVERLESS.md).
- Terminal profile capture required two corrections: the API's 250-event page limit, then matching redacted SQL history by exact job-task ID. Earlier captures are retained. The corrected capture has 40 queries, including 16 pipeline refresh queries: 7,008 Photon task ms / 183,699 total task ms = 3.81%. Per-matching-stage attribution and every executed fallback operator remain unavailable; the attempted export route returned `ENDPOINT_NOT_FOUND`. Billing is unreconciled. [Photon report](bench/PHOTON.md), [corrected capture](experiments/serverless-dqx-profile-v2.json). The planned iteration-5 fixture exports committed snapshots for independent journal replay after FEVM expiry; it is still unexecuted.
- The official Affiliations citation was recovered: Aumueller/Rahm 2009 Table 3 reports F1 0.832 (web URL overlap) and 0.442 (Soft TF-IDF with location). Its 2,450-variant/670-institution evaluation differs from the downloadable 2,260-record/330-cluster corpus. The inspected FAMER paper does not evaluate Affiliations. [Extracted figures, archived-source hashes and comparability limits](bench/AFFILIATIONS_REFERENCE.md); no direct comparison is claimed.
- Final ZR-3/4 iteration 8 is predeclared in [ACCEPTANCE_PLAN.md](bench/ACCEPTANCE_PLAN.md), pending active remote work. Source-compatibility replay and benchmark orchestration helpers are prepared; `bench --all --plan` now executes without an engine config, but the actual campaign has not run. The declaration has not been sealed, defaults have not changed, and no replay has run. Historical fixtures explicitly pin gram/scalar settings before default adoption. Forty-one pure config/CLI/replay/evidence checks pass. The original freeze remains unchanged. A ninth iteration and a repeat of the million-record failure are not authorized.

- Native/all-paid-off run `622968507486615` passed all three tasks in 1,577.39s including runner deployment/capture. F1 deltas are exactly zero for both FEBRL variants; seeded quarantine is exact. Job terminal and pipeline IDLE, cleanup errors empty. Iteration-5 fixture is active as run `180393856450383`, job `1048559310465200`, experiment `20260920T040114Z-serverless-cluster-fixture-bc35c5`.
- Selected defaults are now MinHash, IDF token cosine and the measured 30 clustering rounds. [Static config proof](bench/config_defaults_compatibility.json) verifies unchanged config logic and identical expanded eight frozen/four remote configurations; it does not replace dynamic replay. The original freeze is unchanged. Final replay declaration and execution remain pending. Cluster replay now reloads the two original hash-bound models and thresholds without refitting; the campaign runner blocks the retained failed million tier.
- Corrected Photon query evidence is sealed in `20260920T040102Z-serverless-profile-dqx-audit-b7cd9a`; the earlier local audit failed only because sandbox process inventory denied `ps`, and is retained. Public docs describe query-profile UI JSON Download; no supported REST export was established. [Research receipt](experiments/query-profile-export-research.json). Billing-table metadata is visible on the selected workspace; no billing records have yet been read.

- ZR-6 iteration 5 failed: training task timed out (225s setup, 268s execution; 480s limit), so clustering was skipped. Notebook export reached MLflow model logging and retained a driver-local serving-example validation warning; the warning is not established as the timeout cause. Job terminal, and one precisely inventoried abandoned training scratch table was deleted with absence verified. [Diagnosis](experiments/cluster-fixture-timeout-diagnosis.json), [cleanup](experiments/cluster-fixture-timeout-cleanup.json).
- Iteration 6 is predeclared in [the Delta plan](bench/DELTA_PUBLICATION_PLAN.md): unchanged data/model/assertions, durable progress files and existing tracking-harness volume staging; 600s training, 900s clustering, 1800s job and 3000s outer bound. It has not started. First, bounded read-only evidence capture `20260920T041602Z-query-billing-evidence-4b9f55` is active on newly owned warehouse `ec3b6df6c1cabcd4`; shared warehouse remains untouched. [Query plan](bench/QUERY_EVIDENCE_PLAN.md).

- Query/billing capture ended with all 16 exact query IDs but redacted statement text and no attributed billing rows. Its initial warehouse-stop verification failed; explicit CLI stop and a subsequent read verified owned warehouse `ec3b6df6c1cabcd4` STOPPED. The shared warehouse remains untouched. [Cleanup receipt](experiments/query-evidence-cleanup.json). No zero-cost or per-stage operator claim follows.
- ZR-6 iteration 6 is active as run `994862043938979`, experiment `20260920T042153Z-serverless-cluster-fixture-9608e7`, with the predeclared 600/900/1800-second task/job bounds.

- ZR-6 iteration 6 passed: run `994862043938979` completed training (226s setup, 257s execution) and a separate cluster task. Independent exported-snapshot audit verifies three commits, exact journal/event reconciliation, stable IDs, retry reuse and interrupted-write recovery. Job terminal, cleanup errors empty; outer wall 846.98s. All three serverless capability receipts now audit successfully; Photon operator profiles and attributed billing remain missing.
- The separate source compatibility declaration is sealed in `bench/source_compatibility.json`; all eight frozen models and two clustering reference model trees retain their hashes. ZR-3/4 final iteration 8 is now executing through the actual `lakematch bench --all` command. The million-tier guard remains active. Coherent ZR-1/2/5 refresh is predeclared in [REFRESH_PLAN.md](bench/REFRESH_PLAN.md).

- ZR-3 iteration 8 completed all eight frozen-model replays with zero probability/metadata deltas and identical decisions. Fresh FEBRL CLI times: 30.67s all-field and 27.31s SSN-hidden, with unchanged F1. Original 5k×5k diagnostic independently audited: pipeline F1 0.9881614894 in 35.43s, candidate recall 0.9766; gram nearest-neighbour F1 1.000 in 60.69s including Spark. Both clustering replays pass exact edge/membership comparisons without fitting new models.
- Actual `lakematch bench --all` ran those stages, then exited 2 at the predeclared scale guard ([receipt](bench/campaign_execution.json)). ZR-3 has exhausted eight iterations and remains blocked on the million-record heap failure/full campaign execution; no retry or resource increase was performed. ZR-4 identity/CLI acceptance continues within iteration 8.

- ZR-4 iteration 8 completed: both clustering models replay exactly; both 1% identity mutations reproduce every prior crosswalk/journal/event; all three recovery tests pass; five fresh cluster CLI calls produce exactly three commits with retry reuse and no head rewind. `verify_clusters.check()` returns no errors. [Exact prior-output receipt](experiments/identity-replay-audit.json). Full phase acceptance awaits the active ZR-1 coherent-source refresh.
