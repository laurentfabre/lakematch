...
Created: 2026-09-19. Status: in progress; ZR-3 classifier iteration 2 completed, compact-feature iteration 3 prepared. ZR-4 Splink baselines passed after two retained harness failures; native clustering comparison is prepared. ZR-1/2/5 passed at commit 6f859f4; affected acceptance evidence needs refresh after candidate integration. Classic compute is confirmed unsupported on the selected workspace; ZR-9 parked at ENV.
...
- ZR-4 baseline preparation exposed two failures: the launcher dereferenced the isolated interpreter symlink, then a shared Splink SQL cache returned training records from a new Linker. Both failed runs are retained. The launcher now preserves the virtual environment, and validation uses a fresh DuckDB connection/API plus an endpoint audit before metrics. Count these as iterations 1 and 2; the corrected comparison is iteration 3/8.
- Next eligible work: ZR-3 retrieval iteration 1 has complete measured results; its compatibility checks require a final refresh after the additive runtime truncation option. Run compact-feature iteration 3 from [COMPACT_PLAN.md](bench/COMPACT_PLAN.md), followed by the retrieval/F1 and latency harness,
