#!/usr/bin/env python3
"""Summarize validation trials and paired macro uncertainty without promotion."""
import hashlib
import json
import tarfile
...
DEPS = {'tools/run_methods.py', 'tools/spark_event_metrics.py', 'tools/offline.sb', 'tools/offline_run.py', 'bench/PROTOCOL.md', 'bench/CLASSIFIER_PLAN.md'}
...
def macro(reports):
    ...
    reference = 'levenshtein__gbt'
    return {variant: {'macro_f1': point[variant], 'f1_95ci': interval(draws[variant]),
        'paired_delta_vs_levenshtein_gbt_95ci': interval(draws[variant] - draws[reference]),
...
def main():
    ...
    for line in (ROOT / 'experiments/runs.jsonl').read_text().splitlines():
        ...
        if not event['kind'].startswith('methods-'):
            continue
    ...
    lines = ['# Classifier, string and cardinality comparisons', '',
    ...
    for name in EXPECTED:
        ...
        assert len(report['rows']) == 9 * len(report['cardinalities'])
        ...
        for row in report['rows']:
            ci = row['f1_95ci']
            lines.append(f"| {row['similarity']} | {row['estimator']} | {row['cardinality']} | {row['f1']:.4f} [{ci[0]:.4f}, {ci[1]:.4f}] | {row['precision']:.4f} | {row['recall']:.4f} | {row['threshold']:.2f} | {row['feature_seconds']:.2f}/{row['train_seconds']:.2f}/{row['score_seconds']:.2f} |")
        lines += ['', *[f"- Baseline {r['name']}: F1 {r['f1']:.4f}, threshold {r['threshold']:.2f}, {r['score']}." for r in report['baselines']], '']
    if not index['missing']:
        index.update(status='classifier_comparison_completed', macro=macro(reports))
        ...
        for name, row in index['macro'].items():
            ci = row['f1_95ci']; delta = row['paired_delta_vs_levenshtein_gbt_95ci']
    ...
    (ROOT / 'bench/CLASSIFIERS.md').write_text('\n'.join(lines) + '\n')
    (ROOT / 'bench/methods_index.json').write_text(json.dumps(index, indent=2) + '\n')
