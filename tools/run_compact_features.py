#!/usr/bin/env python3
"""Validation-only comparison of compact native feature sets, with composite tracking."""
import argparse
...
FEATURE_SETS = {
    "native_all": ["idf_token_cosine", "gram_overlap", "monge_elkan_token"],
    "idf_only": ["idf_token_cosine"],
    "scalar_fields": [],
}
...
def main():
    ...
    out = (Path("data/compact_features") / corpus.name).resolve()
    ...
    raw = {"entity": {"name": corpus.name, "fields": corpus.fields},
        ...
        "mlflow": {"tracking_uri": f"sqlite:///{out}/mlflow.db", "experiment": "zr3-compact-features"}}
    config = from_dict(raw)
    report = {"status": "running", "iteration": 3, "corpus": corpus.name,
              "manifest": json.loads(manifest.read_text()), "plan": "bench/COMPACT_PLAN.md",
    ...
    try:
        ...
        with Materializer(spark, config, capabilities) as m:
            ...
            reference = None
            for feature_set, token_features in FEATURE_SETS.items():
                similarity = "levenshtein"
                cfg_raw = deepcopy(raw)
                cfg_raw["features"].update(string_similarity=similarity, udf_features=False, multi_token=token_features)
                ...
                with Materializer(spark, cfg, capabilities) as fm:
                    ...
                    plan_path = out / f"{feature_set}.plan.txt"
                    ...
                    for estimator in [args.estimator]:
                        ...
                        for policy in report["cardinalities"]:
                            ...
                            rows.append({"variant": variant, "similarity": similarity, "estimator": estimator, "token_features": token_features,
                        ...
                        model_record = tracking.log_composite(model, frozen, labels=consumed, input_example=example,
                            experiment="zr3-compact-features", staging_root=out / "staging", idf_path=str(idf_path) if "idf_token_cosine" in token_features else None,
