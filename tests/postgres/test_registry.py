...
def test_migrations_are_checksumming_retry_safe_and_reject_downgrade(registry, postgres, tmp_path):
    with postgres.connect() as connection:
        assert list(apply_migrations(connection, MIGRATIONS)) == [1, 2, 3]
        assert connection.execute("SELECT count(*) FROM lm_control.schema_migration").fetchone()[0] == 3
        for path in MIGRATIONS.glob("*.sql"):
            shutil.copy2(path, tmp_path / path.name)
        path = tmp_path / "0003_identity.sql"
